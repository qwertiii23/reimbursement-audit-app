...
func (r *OCRRepository) UpdateInvoice(ctx context.Context, invoice *ocr.Invoice) error {
	...
	result := r.client.GetDB().WithContext(ctx).Model(invoice).
		...
		Updates(map[string]interface{}{
			...
			"batch_number":        invoice.BatchNumber,
			"valid_from":          invoice.ValidFrom,
			"valid_to":            invoice.ValidTo,
			"is_electronic":       invoice.IsElectronic,
			"is_duplicate":        invoice.IsDuplicate,
			"related_invoice_id":  invoice.RelatedInvoiceID,
			"verification_status": invoice.VerificationStatus,
			"verification_time":   invoice.VerificationTime,
