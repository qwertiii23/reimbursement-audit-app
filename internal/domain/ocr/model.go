...
type InvoiceInfo struct {
	...
	TotalWithTax       float64 `json:"total_with_tax"`        // 价税合计(小写)
	TotalAmountInWords string  `json:"total_amount_in_words"` // 价税合计(大写)
	SubtotalAmount     float64 `json:"subtotal_amount"`       // 小计金额
	SubtotalTax        float64 `json:"subtotal_tax"`          // 小计税额

	// 购方信息
	BuyerName      string `json:"buyer_name"`       // 购买方名称
	BuyerTaxNumber string `json:"buyer_tax_number"` // 购买方统一社会信用代码/纳税人识别号

	// 销方信息
	SellerName      string `json:"seller_name"`       // 销售方名称
	SellerTaxNumber string `json:"seller_tax_number"` // 销售方统一社会信用代码/纳税人识别号
	...
	Items        []InvoiceItem `json:"items"`         // 商品明细
}

// InvoiceItem 发票商品明细
type InvoiceItem struct {
	Name             string  `json:"name"`               // 商品/服务名称
	Specification    string  `json:"specification"`      // 规格型号
	Unit             string  `json:"unit"`               // 单位
	Quantity         float64 `json:"quantity"`           // 数量
	UnitPrice        float64 `json:"unit_price"`         // 单价
	AmountWithoutTax float64 `json:"amount_without_tax"` // 不含税金额
	TaxRate          float64 `json:"tax_rate"`           // 税率
	TaxAmount        float64 `json:"tax_amount"`         // 税额
	LineNo           int     `json:"line_no"`            // 行号
}
...
type Invoice struct {
	...
	Items           string     `json:"items" gorm:"type:json;column:items"`                                                                  // 商品明细(JSON)
	TotalItems      int        `json:"total_items" gorm:"type:int;default:0;column:total_items"`                                             // 商品总数
	MainCommodity   string     `json:"main_commodity" gorm:"type:varchar(200);column:main_commodity"`                                        // 主要商品名称
	Status          string     `json:"status" gorm:"type:varchar(20);not null;default:'pending_recognition';column:status"`                  // 状态(待识别/已识别/识别失败)
	...
	Country            string     `json:"country" gorm:"type:varchar(50);default:'China';column:country"`                              // 消费国家
	...
	VerificationStatus string     `json:"verification_status" gorm:"type:varchar(20);default:'unverified';column:verification_status"` // 验证状态
